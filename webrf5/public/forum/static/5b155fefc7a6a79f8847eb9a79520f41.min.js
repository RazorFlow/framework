...
/** File: application/3.0.2/modules/helpdesk/assets/desktop/js/admin.js **/
    ...
    helpdesk: {
        controllers: {
            adminTicket: {
                index: function () {
                    App.onReady(function () {
                        var generalFrom = $('#helpdesk-form-general');
                        generalFrom.on('change', ':radio', function () {
                            var dependentElement = generalFrom.find('.content-tabs-object.' + this.name);
                            if (dependentElement.length) {
                                return parseInt(this.value, 10) ? dependentElement.removeClass('hidden') : dependentElement.addClass('hidden');
                            }
                        });

                        var inputHide = function () {
                            var select = $(this),
                                input = select.siblings('input, select, div'),
                                conArr = ['IS SET', 'IS NOT SET', 'IS EMPTY', 'IS NOT EMPTY', 'ONLY REGISTERED', 'ONLY NOT REGISTERED'];

                            if ($.inArray(this.value, conArr) >= 0) {
                                if (input.hasClass('category-select-ajax') && input.find('select').data('selectAjax')) {
                                    var selectAjaxInline = input.find('select');
                                    selectAjaxInline.select_ajax("destroy");
                                    input.addClass('hidden');
                                    selectAjaxInline.data('selectAjax', false);
                                    selectAjaxInline.attr('disabled', 'disabled');
                                } else {
                                    input.addClass('hidden').attr('disabled', 'disabled');
                                }
                            } else {
                                if (input.hasClass('category-select-ajax') && !input.find('select').data('selectAjax')) {
                                    var selectAjaxInline = input.find('select');
                                    selectAjaxInline.select_ajax({dropdownSelector: '.rollbar-content'});
                                    input.removeClass('hidden');
                                    selectAjaxInline.data('selectAjax', true);
                                    selectAjaxInline.removeAttr('disabled');
                                } else {
                        ...
                        /* Hide/show condition select and input */
                        var conditionsHandler = function () {
                            var checkBox = $(this),
                                wrapper = checkBox.closest('label').siblings('.' + this.id),
                        ...
                        var toggleMailboxList = function () {
                            var form = $(this),
                                filterTo = form.find('[name=filter_to]'),
                                mailboxList = form.find('.mailbox_list');

                            var value = filterTo.filter(':checked').val();
                            if (parseInt(value, 10)) {
                                mailboxList.removeClass('hidden');
                            } else {
                                mailboxList.addClass('hidden')
                                    .find(':checkbox').removeAttr('checked');
                            }
                        };

                        var modifyTicketRow = function (r) {
                            var usersList = $(this),
                            ...
                            usersList.find('.notifications-users').html(result.data.getOutput());
                            usersList.find('.tooltip').powerTip({ placement: 'n', smartPlacement: true });
                            if (result.data.getParam('id')) {
                                var row = usersList.find('#rule_' + result.data.getParam('id'));
                                if (row.length) {
                                    row.closest('li').replaceWith(result.data.getParam('ticket_row'));
                                } else {
                                    usersList.find('.rules_list ul').append(result.data.getParam('ticket_row'));
                                }
                            }
                        ...
                        var addEditRule = function (modal) {
                            var usersList = $(this),
                                modalBody = modal.getContent(true),
                                form = modalBody.find('form'),
                                sourceEmail = App.getParam('source_email');

                            /* disable 'category_type' options 'IS' and 'IS NOT' if 'category_text' hasn't any options */
                            var selectCategory = form.find('#category-category_text');
                            if (!selectCategory.find('option').length) {
                                selectCategory.siblings('select').find('option').each(function () {
                                    if ($.inArray(this.value, ['IS', 'IS NOT']) >= 0) {
                            ...
                            var sourceChange = function () {
                                var form = $(this),
                                    changedVal = form.find('#source').val();
                                if (changedVal == sourceEmail) {
                                    form.find('.mailboxes_checkbox').removeClass('hidden');
                                } else {
                                    form.find('.mailboxes_checkbox').addClass('hidden').find('[name=filter_to][value=0]').trigger(App.event.click());
                                    form.find('.mailbox_list').addClass('hidden');
                                }
                            };
                            form.find('.conditions :checkbox').each(conditionsHandler);
                            form.on('change', '.conditions :checkbox', conditionsHandler);

                            var selectInput = form.find('.select-with-input');

                            selectInput.find('select:first').each(inputHide);
                            selectInput.on('change', 'select:first', inputHide);

                            toggleMailboxList.call(form);
                            form.on('change', '.mailboxes_checkbox :radio', toggleMailboxList.bind(form));

                            /* Add/Delete condition row */
                            selectInput.on(App.event.click(), '.iplus',function () {
                                var $this = $(this),
                                    selectThis = $this.closest('.select-with-input');
                                var ajaxSelect = selectThis.find('.category-select-ajax select');
                                if (ajaxSelect.length) {
                                    ajaxSelect.select_ajax('destroy');
                                    ajaxSelect.parent().addClass('hidden');
                                }
                                var clone = selectThis.clone(true);
                                clone.find('select, :text:not(.select_ajax-input)').each(function () {
                                    var $this = $(this);
                                    $this.find('option').removeProp('selected');
//                                    $this.select_ajax('destroy');
                                    $this.prop('value', null);
                                    this.name = this.name.replace(/[0-9]+(?!.*[0-9])/, function (match) {
                                        return parseInt(match, 10) + 1;
                                    });
                                });

                                selectThis.after(clone);
                                selectThis.parent().find('select').each(inputHide);
                                $this.removeClass('iplus').addClass('idelete');
                                if (ajaxSelect.length) {
                                    console.log(ajaxSelect);
                                    ajaxSelect.select_ajax({dropdownSelector: '.rollbar-content'});
//                                    ajaxSelect.parent().removeClass('hidden');
                                }
                            }).on(App.event.click(), '.idelete', function () {
                                $(this).closest('.select-with-input').remove();
                            });

                            sourceChange.call(form);
                            form.on('change', '#source', sourceChange.bind(form));

                            /* ADD NEW ACTION */
                            var actionRow = form.find('.ticket-action-row:first').clone(),
                            /* Add new action selects */
                                addNewActionRow = function (e) {
                                    e.preventDefault();
                                    var $this = $(this),
                                        cloneRow = actionRow.clone(),
                                        actionSelect = cloneRow.find('.action');
                                    cloneRow.removeClass('hidden')
                                        .find('.notification').remove();
                                    actionSelect.val(1).removeAttr('disabled');
                                    $this.before(cloneRow);
                                };

                            form.on(App.event.click(), '#add_new_action_link', addNewActionRow);
                            /* SHOW/HIDE USER NOTIFICATION SELECT */
                            var toggleNotificationSelect = function () {
                                var select = $(this);
                                if ($.inArray(parseInt(this.value, 10), [3, 4, 5]) < 0) {
                                    select.siblings('div, select').remove();
                                    return true;
                                }
                                if (select.siblings('.ddOutOfVision').length) {
                                    return true;
                                }
                                if (!select.siblings('.notification').length) {
                                    select.after(actionRow.find('.notification').clone());
                                }

                                select.siblings('.notification').customSelect();
                            };
                            form.find('.action:not(.hidden)').each(toggleNotificationSelect);
                            form.on('change', '.action', toggleNotificationSelect);

                            /* REMOVE ACTION */
                            form.on(App.event.click(), '.idelete', function (e) {
                                e.preventDefault();
                                var row = $(this).closest('.ticket-action-row');
                                /* if delete last row */
                                if (!row.siblings('.ticket-action-row').length) {
                                    var notSel = row.find('.notification');
                                    if (notSel.length) {
                                        notSel.data("dd").destroy();
                                        notSel.remove();
                                    }
                                    row.find('.action').attr('disabled', true).val(1);
                                    row.addClass('hidden');
                                } else {
                                    row.remove();
                                }
...
//                            form.find('select.notification').customSelect();
//                            /* hide/show a list of quick options */
//                            form.on(App.event.click(), ".notification-edit-options li", function () {
//                                form.find(".notification-select").val($(this).find('span').attr('notification'));
//                                $(this).closest('.list-options').hide();
//                            });
//                            form.find(".notification-edit-options").hover(function (e) {
//                                return e.type === 'mouseenter' ? modalOptions.show() : modalOptions.hide();
//                            });

//                            /* change notification user group */
//                            form.on('change', '#assignment', function () {
//                                var select = this;
//                                form.find('.notification-edit-table').each(function () {
//                                    var table = $(this);
//                                    if (table.data('value') == select.value) {
...
//                            });
                            ...
                            form.on('submit', function (e) {
                                ...
                                App.remote.post(form.attr('action'), form.serialize(), function (r) {
                                    ...
                                    button.toggleDisable();
                                    modifyTicketRow.call(usersList, r);
                                    rulesList.find(':radio:checked').trigger('change');
                                    App.dynamic.modal.close(modal.modal);
                                });
                            });
                        };

                        var notificationContent = $('.notifications-content'),
                            rulesList = notificationContent.find(".rules_list"),
                            usersList = notificationContent.find('.notifications-users');

                        /* Handler for ADDING a "Ticket Rule"*/
                        $('#add_new_filter').openFromUrl(addEditRule.bind(notificationContent));

                        /* Handler for EDITING a "Ticket Rule"*/
                        rulesList.openFromUrl({modal: {title: App.t("Edit Ticket Rule")}}, '.iedit', addEditRule.bind(notificationContent));


                        /* When we select a ticket rule from the the thicket rule list */
                        rulesList.on('change', ':radio', function () {
                            var radio = $(this),
                                preloader = $("<i class='ico-small preloader'></i>");
                            radio.addClass('hidden').after(preloader);

                            App.remote.post(App.baseUrl('admin/helpdesk/ticket-rules/select'),
                                {id: this.id.split('rule_')[1]}, function (r) {
                                    preloader.remove();
                                    radio.removeClass('hidden');
                                    modifyTicketRow.call(notificationContent, r);
                                }
                            );
                        });

                        /* EDIT user notification */
                        usersList.openFromUrl({modal: { title: App.t("Notifications Settings")}}, '.iedit', function (modal) {
                            var modalBody = modal.getContent(true),
                                modalOptions = modalBody.find('.list-options');

                            modalBody.on('change', 'select[name="user_id"]', function () {
                                var selected = modalBody.find("select[name='user_id'] option:selected"),
                                    privateCat = modalBody.find(".notification-edit-table tr[is_private=1]");

                                if (!parseInt(selected.attr("see_private"), 10)) {
                                    privateCat.attr('disabled', 'disabled').hide();
                                } else {
                                    privateCat.removeAttr("disabled").show();
                                }
                            });

                            modalBody.find(".notification-edit-options").hover(function (e) {
                                return e.type === 'mouseenter' ? modalOptions.show() : modalOptions.hide();
                            });

                            modalBody.on(App.event.click(), ".notification-edit-options li span", function () {
                                var sel = $(this).attr('notification');
                                modalBody.find("tbody option[value=" + sel + "]").prop('selected', true);
                                modalOptions.hide();
                            });
                            ...
                            modalBody.on("submit", "form", function (e) {
                                ...
                                App.remote.post(this.action, $(this).serialize(), function (r) {
                                    var result = App.remote.parseResult(r);
                                    if (result.data.isError()) {
                                        App.notify.error(result.data.getMessage());
                                        return false;
                        ...
                        /* DUPLICATE RULE */
                        rulesList.on(App.event.click(), '.iduplicate', function (e) {
                            e.preventDefault();
                            var defaultName = $(this).parents('li').find('label')[0].textContent,
                                newName = prompt(App.t('Enter Title of Duplicate'), $.trim(defaultName));
                            if (!newName) {
                                return false;
                            }
                            App.remote.post(this.href, {name: newName}, function (r) {
                                var result = App.remote.parseResult(r);
                                if (result.data.isError()) {
                                    App.notify.error(result.data.getMessage());
                                    return false;
                        ...
                        /* Handler for 'click' on MAIL ICON to change a notification state */
                        usersList.on(App.event.click(), '.notifications-link i, .change-notify a', function (e) {
                            e.preventDefault();
                            var $this = $(this),
                                wrapper = $this.closest('li[data-info]'),
                                data = {};
                            data.info = wrapper.data('info');
                            var preLoader;
                            if ($this.is('a')) {
                                data.info.state = $this.data('state');
                                preLoader = $this.closest('.department-notification').find('.ico-small');
                            } else {
                                preLoader = $(this);
                            }
                            preLoader.addClass('preloader');
                            data.info.rule_id = rulesList.find(':radio:checked').attr('id').split('rule_')[1];
                            App.remote.post(App.baseUrl('admin/helpdesk/ticket-rules/change-state'), data, function (r) {
                                var result = App.remote.parseResult(r),
                                    info = result.data.getParam('info');

                                usersList.find('li[data-info]').each(function () {
                                    var $this = $(this),
                                        rowInfo = $this.data('info');

                                    if ($.isPlainObject(info[rowInfo.user_id])) {
                                        rowInfo.state = info[rowInfo.user_id].state;
                                        rowInfo.id = info[rowInfo.user_id].id;
                                        $this.data('info', rowInfo);

                                        $this.find('span')[0].className = info[rowInfo.user_id]['class'];
                                        $this.find('i').data('powertip', App.t(info[rowInfo.user_id].title));
                                    }
                                });
                        ...
                        /* Handler for DELETING a "Ticket Rule"*/
                        rulesList.on(App.event.click(), '.idelete', function (e) {
                            e.preventDefault();
                            var self = $(this),
                                row = self.closest('li');
                            if (confirm(App.t("Are your sure you want to delete this rule"))) {
                                self.removeClass('idelete').addClass('preloader');
                                App.remote.get(this.href, null, function (r) {
                                    var result = App.remote.parseResult(r);
                                    self.addClass('idelete').removeClass('preloader');
                                    if (result.data.isError()) {
                                        App.notify.error(result.data.getMessage());
                                        return false;
                                    }
                                    if (row.find(':radio:checked').length) {
                                        row.siblings(':eq(0)').find(':radio').trigger('change');
                                    }
                                    row.remove();
                                    App.notify.success(result.data.getMessage());
                                });
                        ...
                        /* Handler for SORTING a "Ticket Rule"*/
                        rulesList.find('ul').sortable({
                            handle: ".imove",
                            axis: 'y',
                            update: function (event, ui) {
                                var position = $(this).find('li').map(function () {
                                    return $(this).find('input:radio')[0].id.split('rule_')[1];
                                }).get();
                                ui.item.find("a.imove").removeClass("imove").addClass('preloader');
                                App.remote.post(window.location.href, {position: position}, function (r) {
                                    var result = App.remote.parseResult(r);
                                    ui.item.find("a.preloader").addClass("imove").removeClass('preloader');
                                    if (result.data.isError()) {
                                        App.notify.error(result.data.getMessage());
                        ...
                        rulesList.openFromUrl('.iview');

                        /* TEST RULES */
                        $('#test_rules').openFromUrl({modal: {title: App.t("Test Rules")}}, function (modal) {
                            var modalBody = modal.getContent(true),
                                form = modalBody.find('form');
                            form.on('submit', function (e) {
                                e.preventDefault();
                                var back = form.find('#back_test'),
                                    submitButton = form.find('#submit_test'),
                                    modalScrollable = form.find('.modalScrollable');

                                var pattern = /^[\w.\-_]+@[\w.\-_]+$/;
                                if (!pattern.test($('#notify_email').val().trim())) {
                                ...
                                form.find('.test-results').remove();
                                back.addClass('hidden');
                                form.find('.modalBlock').addClass('hidden');
                                var preloader_block = $('<div></div>').addClass('modal-preloader-block').css({
                                    background: 'url("' + App.assets.common.image('/modal_preloader.gif') + '") no-repeat scroll center center',
                                    width: '100%',
                                    height: '100px'
                                });
                                form.find('.modalBlock:last').after(preloader_block);
                                $(window).resize();
                                App.remote.post(this.action, $(this).serialize(), function (r) {
                                    var result = App.remote.parseResult(r);
                                    form.find('.modal-preloader-block').remove();
                                    if (result.data.isError()) {
                                        form.find('.modalBlock').removeClass('hidden');
                                        $(window).resize();
                                        App.notify.error(result.data.getMessage());
                                        return false;
                                    }
                                    form.find('.modalBlock:last').after(result.data.getOutput());
                                    modal.setTitle(App.t("Test Rules - Results"));
                                    back.removeClass('hidden');
                                    submitButton.addClass('hidden');
                                    $(window).resize();
                                });
                            });
                            form.on(App.event.click(), '#back_test', function () {
                                $(this).addClass('hidden');
                                form.find('.modalBlock').removeClass('hidden').filter('.test-results').remove();
                                form.find('#submit_test').removeClass('hidden');
                                modal.setTitle(App.t("Test Rules"));
                            });
                        });
                    });
                }
            ...
            adminEmailConflicts: {
                index: function () {
                    App.onReady(function () {
                        ...
                        $("#clean-log").on(App.event.click(), function () {
                            var self = $(this);
                            var wrapper = self.closest('tr');
                            wrapper.removeClass("error");
                            var preloader = self.closest('div').find('i.preloader');
                            if (confirm(App.t('Are you sure? You can\'t UNDO this operation!'))) {
                                preloader.show();
                                App.remote.get(self.attr("href"), {format: 'json'}, function (r) {
                                    var result = App.remote.parseResult(r);
                                    if (result.data.isError()) {
                                        App.notify.error(result.data.getMessage());
                                        return false;
...
App.route().on('admin_mailing', function () {
    /* Email Conflicts JS*/
    var conflictContent = $('.email_conflicts');
    conflictContent.openFromUrl({modal: {
        title: App.t("Email Conflict")
    }}, '.iview');

    conflictContent.on(App.event.click(), '.idelete', function (e) {
        e.preventDefault();
        var self = $(this);
        if (confirm(App.t("Are your sure you want to delete this conflict"))) {
            self.removeClass('idelete').addClass('preloader');
            App.remote.get(this.href, null, function (r) {
                var result = App.remote.parseResult(r);
                self.addClass('idelete').removeClass('preloader');
                ...
                var delRow = self.closest('tr');
                if (!delRow.siblings('tr').length) {
                    self.find('table').addClass('hidden');
                    self.find('.result-not-found').removeClass('hidden');
                }
                delRow.remove();
                App.notify.success(result.data.getMessage());
            });
    ...
    /* Mailboxes JS*/
    /* test connection attach event */
    var testConnection = function (callback) {
        var form = $(this),
            url = App.baseUrl('admin/mailing/incoming-mailbox/test-connection'),
        ...
        return $.ajax({
            ...
            data: form.serialize(),
            type: 'POST'
        }).done(callbackFunc.bind());
    };

    var addEditMailbox = function (modal) {
        var self = $(this),
            modalBody = modal.getContent(true),
            form = modalBody.find('form');

        form.on(App.event.click(), '#test_connection', testConnection.bind(form));

        form.on('submit', function (e) {
            e.preventDefault();
            var preloader = form.find(".preloader");
            testConnection.call(form, function () {
                App.remote.post(form.attr('action'), form.serializeArray(), function (r) {
                    var result = App.remote.parseResult(r);
                    preloader.addClass('hidden');
                    if (result.data.isError()) {
                        App.notify.error(result.data.getMessage());
                    ...
                    /* Checks if we are creating the first mailbox */
                    if (self.find('table').hasClass('hidden')) {
                        self.find('table').removeClass('hidden');
                        self.find('.result-not-found').addClass('hidden');
                    }
                    var row = self.find('#mailbox_' + result.data.getParam('id'));
                    if (row.length) {
                        row.replaceWith(result.data.getOutput());
                    } else {
                        self.find('tbody').append(result.data.getOutput());
                    }
                    App.notify.success(result.data.getMessage());
                    App.dynamic.modal.close(modal.modal);
                });
    ...
    var mailboxContent = $(".mailbox");

    /* binds functions to links for add and edit mailbox */
    $('#add_new_mailbox').openFromUrl({
        modal: {
            title: App.t("New Mailbox"),
            width: 600,
            shader: false
        }
    }, addEditMailbox.bind(mailboxContent));

    mailboxContent.openFromUrl({
        modal: {
            title: App.t("Edit Mailbox"),
            width: 600,
            shader: false
        }
    }, '.iedit', addEditMailbox.bind(mailboxContent));

    /* enable or disable mailbox */
    mailboxContent.on('change', '.toggle_action', function () {
        var preloader = App.preloader(this),
            type = $(this).closest('table').data('type');
        preloader.start();
        App.remote.post(App.baseUrl('admin/mailing/incoming-mailbox/toggle'), {id: this.value, type: type}, function (r) {
            var result = App.remote.parseResult(r);
            preloader.stop();
            if (result.data.isError()) {
                App.notify.error(result.data.getMessage());
    ...
    mailboxContent.on(App.event.click(), '.idelete', function (e) {
        e.preventDefault();
        var self = $(this);
        if (confirm(App.t("Are your sure you want to delete this mailbox"))) {
            self.removeClass('idelete').addClass('preloader');
            App.remote.get(this.href, null, function (r) {
                var result = App.remote.parseResult(r);
                self.addClass('idelete').removeClass('preloader');
                ...
                var delRow = self.closest('tr');
                if (!delRow.siblings('tr').length) {
                    mailboxContent.find('table').addClass('hidden');
                    mailboxContent.find('.result-not-found').removeClass('hidden');
                }
                delRow.remove();
                App.notify.success(result.data.getMessage());
            });
