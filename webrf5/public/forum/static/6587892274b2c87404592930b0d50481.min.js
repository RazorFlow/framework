...
/** File: application/3.0.2/modules/chat/assets/desktop/js/admin.js **/

App.onReady(function () {
    "use strict";
    var lvChatChange = function () {
        $("#lv-chat-text").text($("#chat_widget_display_name").val());
        $('.chat-widget-demo').css('background-color', '#' + $("#chat_widget_background_color").val());
        $('.chat-widget-demo').css('border-color', '#' + $("#chat_widget_border_color").val());
        if ($("#chat_widget_corners").is(":checked")) {
            $('.chat-widget-demo').addClass("lv-head-round");
        } else {
            $('.chat-widget-demo').removeClass("lv-head-round");
        }
        if ($("#chat_widget_drop_shadow").is(":checked")) {
            $('.chat-widget-demo').addClass("lv-box-shadow");
        } else {
            $('.chat-widget-demo').removeClass("lv-box-shadow");
        }
        if ($("input[name=chat_widget_location]:checked").val() == "left") {
            $('.chat-widget-demo').addClass("lv-leftPos");
        } else {
            $('.chat-widget-demo').removeClass("lv-leftPos");
        }
    };

    var chatForm = $('#chat-form-widget');
    chatForm.on('change', 'input', function () {
        App.remote.post(chatForm.attr("action"), chatForm.serialize(), function (r) {
            var result = App.remote.parseResult(r);
    ...
    $("#chat_widget_display_name").keyup(function () {
        lvChatChange();
    });

    $("#chat_widget_background_color").change(function () {
        lvChatChange();
    });

    $("#chat_widget_border_color").change(function () {
        lvChatChange();
    });

    $("#chat_widget_corners").change(function () {
        lvChatChange();
    });

    $("#chat_widget_drop_shadow").change(function () {
        lvChatChange();
    });

    $("input[name=chat_widget_location]").change(function () {
        lvChatChange();
    });

    lvChatChange();

    $(".chat-result-area div.result-code")
        .css('height', '330px')
        .css('font-size', '14px')
        .text("<!-- START USERESPONSE CHAT -->\n\
<script type='text/javascript'>\n\
var jabberServer = '" + App.getParam('jabberServer', '/') + "';\n\
var chatBaseUrl = '" + App.baseUrl() + "';\n\
scriptElem = document.createElement('script');\n\
scriptElem.setAttribute('type','text/javascript');\n\
scriptElem.setAttribute('src', '" + App.baseUrl('/public/chat/init_chat.js') + "');\n\
document.getElementsByTagName('head')[0].appendChild(scriptElem);\n\
</script>\n\
<!-- END USERESPONSE CHAT -->");

    $("#chat_open_popup_or_widget").change(function () {
        var link = $('#chat_link');
        if ($(this).val() == 'window') {
            if (!link.length) {
                $(this).after('&nbsp;<a id="chat_link" href="#">' + App.t('Chat Link') + '</a>');
            }
        } else {
            if (link.length) {
                link.remove();
    ...
    $("#chat_open_popup_or_widget").change();

    $('.content-tabs-area.chat').on('click', '#chat_link', function (event) {
        event.preventDefault();
        alert('javascript:openLvChat();');
    });
});
    ...
    chat: {
        controllers: {
            admin: {
                index: function () {
                    App.onReady(function () {
                        $("input[name=chat_introduction_message_yes_no]").click(function () {
                            if ($(this).val() == 1) {
                                $(".chat-block").show();
                            } else {
                                $(".chat-block").hide();
                        ...
                        $("input[name=chat_introduction_message_yes_no]:checked").click();

                        $("input[name=chat_auto_open]").click(function () {
                            var block = $('#chat_auto_open_seconds').closest('div.hidden-block');
                            if ($(this).val() == 1) {
                                block.show();
                            } else {
                                block.hide();
                        ...
                        $("input[name=chat_auto_open]:checked").click();

                        var main = $('#main');
                        main.on(App.event.click(), ".account-form", function (event) {
                            event.preventDefault();
                            var title = $(this).attr('id') == 'add-new-chat-account' ? 'New XMPP Account' : 'Edit XMPP Account';
                                ...
                                complete: function (modal) {
                                    ...
                                    form.on("submit", function (e) {
                                        ...
                                        App.remote.post(form.attr("action"), form.serialize(), function (r) {
                                            ...
                                            App.notify.success(result.data.getMessage());
                                            App.dynamic.modal.close(modal.modal);
                                            $('#accounts-table').html(result.data.getOutput());
                                        });
                        ...
                        main.on(App.event.click(), '.table-actions .idelete', function () {
                            if (confirm(App.t("Are you sure? You can't UNDO this operation!"))) {
                                var td = $(this).closest('.table-actions');
                                td.find('a').addClass('hidden');
                                td.find('.preloader').addClass('ico-small');
