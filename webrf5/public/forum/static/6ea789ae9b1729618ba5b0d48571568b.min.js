...
/** File: application/3.0.2/modules/polls/assets/desktop/js/manage.js **/

App.onReady(function () {

    var wrapper = $('.polls-manage-element').parents('.mainArea:first');
    var fA = $('.filter-area');

    var openEditPopup = function (url, row) {
        App.dynamic.modal.openFromUrl({
            modal: {
                title: App.t("Edit Poll"),
                width: '600',
                shader: false,
                useCustomScroller: true,
                useWrapper: false
            ...
            complete: function (modal) {
                ...
                modalBody.on(App.event.click(), '.add-option', function () {
                    ...
                    optionsWrapper.find('tr.option:last input').attr('value', '').attr('name', 'options[new][]');
                    return false;
                });
                ...
                modalBody.on("submit", 'form', function (e) {
                    ...
                    App.remote.post(form.attr("action"), form.serialize(), function (r) {
                        ...
                        App.notify.success(result.data.getMessage());
                        App.dynamic.modal.close(modal.modal);
                        fA.find('select:first').trigger('change');
                    });
    ...
    var initSortable = function () {
        this.find('tbody').sortable({
            ...
            update: function (event, ui) {
                ...
                App.remote.post(App.baseUrl('/polls/reorder'), {order: order}, function (r) {
                    var result = App.remote.parseResult(r);
                    ui.item.find("a.preloader").addClass("imove").removeClass('preloader');
                    if (result.data.isError()) {
                        App.notify.error(result.data.getMessage());
    ...
    wrapper.on(App.event.click(), '.iedit', function () {
        openEditPopup($(this).attr('href'), $(this).parents('tr:first'));
        return false;
    });

    wrapper.on(App.event.click(), '.idelete', function () {
        if (confirm(App.t("Are you sure? You can't UNDO this operation!"))) {
            var button = $(this);
            var row = button.parents('tr:first');
            button.after('<i class="ico-small preloader"></i>').remove();
    ...
    initSortable.call($('#polls-table'));

    /* toggle featured */
    wrapper.on(App.event.click(), '.featured, .not-featured', function () {
        var self = this,
            $this = $(this),
            tr = $this.closest('tr'),
            isFeatured = $this.hasClass('featured');
    ...
    /* toggle state */
    wrapper.on(App.event.click(), '.iarchive', function () {
        var self = this,
            $this = $(this),
            tr = $this.closest('tr'),
            isNormal = $this.hasClass('state-normal');
        var url = $this.parents('a:first').attr('href');
        self.className = 'ico-small tooltip preloader';
        App.remote.get(url, null, function (r) {
            var result = App.remote.parseResult(r);
            ...
            self.className = self.className.replace('preloader', isNormal ? 'state-archived' : 'state-normal');
            $this.addClass('iarchive');
            $this.data('powertip', isNormal ? App.t('Restore') : App.t('Move to Archive'));
            var status = fA.find('.status-filter').val();
            if (status != '' && status != result.data.getParam('status')) {
                tr.remove();
            } else {
                tr.find('td.state-column').text(isNormal ? App.t('archived') : App.t('normal'));
            }
        });
        return false;
    });

    wrapper.on(App.event.click(), '.iview', function () {
        App.dynamic.modal.openFromUrl({
            modal: {
                title: '',
                width: '600',
            ...
            request: {
                ...
                complete: function (result) {
                    var title = result.data.getParam('caption');
                    var url = result.data.getParam('url', null);
                    if (url != null) {
                        title = $('<a></a>').text(result.data.getParam('caption')).css('text-decoration', 'none').attr('href', url).css('color', '#555555').css('font-size', '16px');
                    }
                    $('div.modal').find('.modalHeader .modalTitle').html(title);
                }
            }
        });
        return false;
    ...
    fA.on('change', 'select', function () {
        var url = App.baseUrl('polls/manage') + '?status=' + fA.find('.status-filter').val() + '&dates=' + fA.find('.dates-filter').val();
        fA.find('.preloader').addClass('ico-small');
        App.remote.get(url, null, function (r) {
            fA.find('.preloader').removeClass('ico-small');
            var result = App.remote.parseResult(r);
            if (result.data.isError()) {
                App.notify.error(result.data.getMessage());
                return false;
            }
            wrapper.html(result.data.getOutput());
            initSortable.call($('#polls-table'));
        });
    });
