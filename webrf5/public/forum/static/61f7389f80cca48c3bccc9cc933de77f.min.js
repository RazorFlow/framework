...
/** File: application/3.0.2/modules/system/assets/desktop/js/frontend.js **/
    ...
    system: {
        controllers: {
            ...
            auth: {
                ...
                login: function () {
                    App.onReady(function () {
                        $('.login-footer').on(App.event.click(), 'a.forgotPass', function (e) {
                            e.preventDefault();
                            var link_id = this.id,
                                form = $(this).closest('form');
                            form[0].reset();
                            form.addClass('hidden');
                            $('.'+link_id+'-form').removeClass('hidden');
                        });

                        $('form.loginForm-form').on('submit', function (e) {
                            var valid = true;

                            $(this).find("input.required").removeClass('error').each(function () {
                                var validOne = $(this).val().trim() != '';
                                valid = valid && validOne;
                                if (!validOne) {
                                    $(this).addClass('error');
                                }
                            });
                ...
                index: function () {
                    App.onReady(function () {
                                ...
                                complete: function (modal) {
                                    ...
                                    modalBody.on('click', 'input[type="submit"]', function (e) {
                                        ...
                                        App.remote.get(link.attr("href"), {format: 'json', reason: modalBody.find('textarea').val()}, function (r) {
                                            ...
                                            $('.user-banned').removeClass('hidden');
                        ...
                        if (App.getParam('openProfileSettings', 0) == 1) {
                            $('a.user-setting:first').trigger('click');
                        }
                        ...
                        $.each(App.getParam('userProfileTabUrls', []), function (id, url) {
                            $('#'+id).on('click', function () {
                                ...
                                App.remote.get(url, {format: 'json'}, function (r) {
                                    ...
                                    wrapper.html('<div class="activity-log">' + result.data.getOutput() + '</div>').data('loaded', 1);
...
//                        });
            ...
            users: {
                _init: function () {
                    App.onReady(function () {
                        $("#search-people").focus(function () {
                            if ($(this).val() == App.t('Search Users')) {
                                $(this).val('');
                            }
                        });
                        $("#search-people").focusout(function () {
                            if ($(this).val() == '') {
                                $(this).val(App.t('Search Users'));
                            }
                        });
                        ...
                        paginator = function () {
                            if ($(".sortByDate").val() == 'default_view'
                                && $(".sortByRole").val() == 'all'
                                && ($("#search-people").val() == '' || $("#search-people").val() == App.t('Search Users'))) {
                            ...
                            if (element.is("select")) {
                                ...
                                filter = element.parent().parent();
                            }
                            filter.find(".preloader").css('display', 'inline-block');
                ...
                index: function () {
                    App.onReady(function () {
                        $('#new-user').on(App.event.click(), function (e) {
                            e.preventDefault();

                            App.dynamic.modal.openFromUrl({
                                modal: {
                                    title: App.t('New User'),
                                    width: '600',
                                    shader: false,
                                    useCustomScroller: true,
                                    useWrapper: false
                                ...
                                request: {
                                    url: App.baseUrl('/users/add')
                                },
                                complete: function (modal) {
                                    var modalBody = modal.getContent(true);
                                    var form = modalBody.find('form');

                                    form.on('change', 'input[name="generate_password"]', function () {
                                        if ($(this).val() == 0) {
                                            form.find('.password-block').removeClass('hidden');
                                        } else {
                                            form.find('.password-block').addClass('hidden');
                                        }
                                    });

                                    form.on('change', '#toggle_welcome_message', function () {
                                        $('#welcome_message').toggleClass('hidden');
                                    });
                                    ...
                                    form.on("submit", function (e) {
                                        e.preventDefault();
                                        var form = $(this),
                                            email = form.find('input[name="email"]:first');

                                        if (/^[\w.\-_]+@[\w.\-_]+$/.test(email.val().trim()) == false) {
                                        ...
                                        form.find('.preloader').css('margin-right', '10px').addClass('ico-small');

                                        App.remote.post(form.attr("action"), form.serialize(), function (r) {
                                            form.find('input[type=submit], button[type="submit"]').removeAttr('disabled');
                                            form.find('.preloader').removeClass('ico-small');
                                            var result = App.remote.parseResult(r);
                                            if (result.data.isError()) {
                                                App.notify.error(result.data.getMessage());
                                                return false;
...
//                        });
                ...
                index: function () {
                    App.onReady(function () {
                        ...
                        $('.b-ur-tabs li').click(function () {
                            ...
                            $(".b-ur-search-input form input.b-ur-text-input").css('color', '#acacac').val(App.t('Enter topic for us to find it'));
            ...
            reports: {
                index: function () {
                    App.onReady(function () {
                        $('#external-scripts').openFromUrl({modal: {title: App.t("External Scripts")}}, function (modal) {
                            var modalBody = modal.getContent(true),
                                form = modalBody.find('form'),
                                submit = form.find('#submit-scripts'),
            ...
            browse: {
                _init: function () {
                    App.onReady(function () {
                        ...
                        var main = $(".mainArea");
                        var preloader = $(".filter-area .preloader");
                        $('.date-pick').datePicker({startDate: '1996-01-01'});

                        //paginator
                        main.on("click", ".paginator li a", function (event) {
                            event.stopImmediatePropagation();
                            var paginator = $(this).parent().parent();
                            paginator.find(".preloader").show();
                            var data = {
                                'format': 'json',
                                'page': $(this).attr("id").match(/\d+/)[0],
                                'type': $(".filter.filter-active").attr("id")
                            };
                            var sortEl = $('.sortBar').find('span.sortUp, span.sortDown,').eq(0);
                            if (sortEl.is('span')) {
                                data.order = sortEl.attr('id');
                                data.sort = 'sortUp' === sortEl.attr('class') ? 0 : 1;
                            }
                        ...
                        main.on("focus blur", "input#text", function (e) {
                            var self = $(this);
                            if (e.type === 'focusin' && self.val() === App.t('Search topics')) {
                                self.val('');
                            }
                            if (e.type === 'focusout' && self.val() === '') {
                                self.val(App.t('Search topics'));
                            }
                        });

                        //sort
                        main.on("click", "tr.sortBar th span", function () {
                            preloader.show();
                            var sort = $(this).hasClass("sortUp") ? 1 : 0;
                            var data = {
                                'order': $(this).attr("id"),
                                'sort': sort
                            };
                        ...
                        $('.filter').on("click", function () {
                            var self = $(this);
                            var form = $('#system-form-browsesearch');
                            preloader.show();
                            self.siblings().removeClass("filter-active").find(".marker").hide();
                            self.addClass("filter-active").find(".marker").show();
                            form.find('input[name=type]').val(self.attr("id"));

                            App.remote.get(location.href, form.serialize(), function (r) {
                                var result = App.remote.parseResult(r);
                                preloader.hide();
                                if (result.data.isError()) {
                                    App.notify.error(result.data.getMessage());
            ...
            moderation: {
                _init : function() {
                    App.onReady(function(){
                        ...
                        $(".paginator li a").live("click", function(){
                            paginator = $(this).parent().parent();
                            paginator.find(".preloader").show();
                            form = $(".filter-area");
                            data = App.getFormParams(form);
                            data['page'] = $(this).attr("id").match(/\d+/)[0];
                            url = location.href;
                            if (url.match(/type[^\/]*/g)) {
                                url = url.replace(/type[^\/]*/g, "type/" + $(".filter-area #type").val());
                            }
                            App.remote.get(url, data, function(r) {
                                ...
                                $(".mainArea-type2").html(result.data.getOutput());
                                $('.table-view .object:odd').addClass('color-obj');
                            });
                            return false;
                        });

                        var declineReason = App.getParam('declineReason', 1);
                        var badgeWrapper = $(this).parents('div.object:first').hasClass('comment') ? $('#tb-moderate-comments') : $('#tb-moderate-topics');
                        var approveDeclineResult = function (object) {
                            object.remove();
                            if (!$('.table-view .object').length) {
                                $(".filter-area-add").hide().prev().show();
                                if (App.getParam("objects") == "responses") {
                                    $('.table-view').html("<p class='result-not-found'>" + App.t('Topics not found') + "</p>");
                                }
                                if (App.getParam("objects") == "comments") {
                                    $('.table-view').html("<p class='result-not-found'>" + App.t('Comments not found') + "</p>");
                                }
                            }
                            if (App.getParam("objects") == "responses") {
                                responses = $(".moderate-responses");
                                if (responses.eq(0).text().match(/\d+/)) {
                                    count = responses.eq(0).text().match(/\d+/)[0];
                                    count -= 1;
                                    text = responses.eq(0).html().replace(/\(\d+\)/, "(" + count + ")");
                                    responses.html(text);
                                }
                            }
                            if (App.getParam("objects") == "comments") {
                                comments = $(".moderate-comments");
                                if (comments.eq(0).text().match(/\d+/)) {
                                    count = comments.eq(0).text().match(/\d+/)[0];
                                    count -= 1;
                                    text = comments.eq(0).html().replace(/\(\d+\)/, "(" + count + ")");
                                    comments.html(text);
                                }
                            }

                            $('.table-view .object:odd').addClass('color-obj');
                            $.each([$('#top-bar-moderate'), badgeWrapper], function () {
                                var count = parseInt($(this).find('span.badge:first').text()) - 1;
                                if (count > 0) {
                        ...
                        $('.approve-delete-block a').live(App.event.click(), function(e){
                            object = $(this).parents(".object");

                            if (declineReason && $(this).hasClass('delete') && !object.hasClass('comment')) {
                                var link = $(this);
                                link.closest('li.activ-pos').removeClass('activ-pos');
                                App.dynamic.modal.openFromUrl({
                                    modal: {
                            ...
                            var wrapper = $(this).parents('.approve-delete-block:first');
                            wrapper.find('*:not(.preloader)').hide();
                            wrapper.find('.preloader').show();
                ...
                topics : function () {
                    App.onReady(function(){
                        $.moderationMassEdit($('.mainArea'), $('.leftarea').find('.sidebar-container'));
                    });
                    App.onReady(function(){
                        $(".sortByModerationType").on('change', function() {
                            var data = App.getFormParams($(".filter-area"));
                            $(this).parents('div.filter-area:first').find(".preloader").css('display', 'inline-block');
                ...
                comments: function () {
                    App.onReady(function () {
                        App.module.system.controllers.moderation.topics();
                    });
                }
            }
        }
...
$(document).ready(function () {
    window.submitForm = function (form_id) {
        if (form_id) {
            $("#" + form_id).unbind("submit").die("submit");
            $("#" + form_id + " input[type=submit], #" + form_id + " button[type=submit]").die("click").unbind("click").click();
        }
    };

    (function ($) {
        var dateFilter = $('select.activity-date-filter');
        var usersFilter = $('select.activity-user-filter');
        var page = 1;
        $(document).on(App.event.click(), 'div.activity-log div.load-more a', function () {
            var wrapper = $('div.activity-log');
            var params = App.getParam('activityLogItemsCount', null);
            var itemsCount = params['items_count'];
            var countPerPage = params['count_per_page'];
            var baseUrl = params['base_url'];

            var button = $(this);
            var preloader = wrapper.find('div.load-more i.preloader');
            button.hide();
            preloader.addClass('ico-small');
            var url = baseUrl;
            ...
            url += '/page/' + ++page;
            var initValue = wrapper.find('div.title-small:last').attr('mode');
            ...
            App.remote.get({
                ...
                success: function (response) {
                    preloader.removeClass('ico-small');
                    button.show();
                    wrapper.find('table.objects-table:last').after(response.data.getOutput());
                    wrapper.find('span.loaded-count').text(page * countPerPage < itemsCount ? page * countPerPage : itemsCount);
    ...
    $.fn.userCard = function (selectors) {
        $(this).on(App.event.click(), selectors, function (e) {
            ...
            App.dynamic.modal.openFromUrl({
                ...
                complete: function (modal) {
                    var modalBody = modal.getContent(true),
                        country = modalBody.find("td[ip]");
                    country.userCardGetCountry();
//                    $.get("http://ipinfo.io/" + country.attr('ip'), function (response) {
 ...
 * @param {jQuery} side
 */
$.moderationMassEdit = function (main, side) {
    var mainArea = main,
        sideArea = side,
        modal = $('.modal'),
        ...
        sidebarArea = $('.leftarea'),
        massEditSidebar = sidebarArea.find('.sidebar-container');
    var topCtrl = $('.ctrl-help').length > 0 ? $('.ctrl-help').offset().top - 70 : 0;

    App.scrollMassEdit(massEditSidebar, topCtrl, 'ctrl-help');
    ...
    var toggleMassEdit = function (action) {
        if (!action) {
            ...
            var topPos = sidebarArea.offset().top - 50;


            App.scrollMassEdit(sidebarArea, topPos, 'mass-edit');
    ...
    if (mainArea) {
        ...
        mainArea.on("click", ".object", function (e) {
            ...
            //target.animate({ opacity: 0.7 }, 30 ).animate({ opacity: 1 }, 30 ).toggleClass('selected');
    ...
    sideArea.on('click', '.links-mass-edit a', function (e) {
        e.preventDefault();
        var link = $(this), topicIds = getObjectIds();
        var oldClass = link.attr('class');

        if (link.hasClass('object_decline') && App.getParam('declineReason', 1) && link.find('input#objects').val() == 'Objects') {
            App.dynamic.modal.openFromUrl({
                modal: {
                    title: App.t("Decline Reason"),
                    width: '500',
                ...
                complete: function (modal) {
                    ...
                    modalBody.on('click', 'input[type="submit"]', function (e) {
                        ...
                        submit.attr('disabled', 'disabled');
                        modalBody.find('.preloader').addClass('ico-small');
                        App.remote.post(form.attr('action'), link.find('input').serializeArray().concat(topicIds).concat({'name':'reason','value':modalBody.find('textarea').val()}), function (r) {
                            var result = App.remote.parseResult(r);
                            if (result.data.isError()) {
                                submit.removeAttr('disabled');
                                modalBody.find('.preloader').removeClass('ico-small');
    ...
    form.on('submit', function (e) {
        e.preventDefault();
        var form = $(this),
            data = form.find('div:not(.hidden) select').serializeArray().concat(getObjectIds());
...
App.onReady(function () {
    // Select / deselect all modules in Moderate Response
...
/** File: application/3.0.2/assets/desktop/js/jquery-ui-datepicker-slider.js **/
...
/** File: application/3.0.2/assets/desktop/js/jquery-ui-timepicker-addon.js **/
...
/** File: application/3.0.2/modules/resources/assets/desktop/js/files.js **/

jQuery.fn.extend({
    attachFiles : function(settings){
        var isSupportHTML5 = $.isFunction(window.FormData) ? 1 : 0,
            mainArea = $(this),
            defaults = {
                extensions: ['jpg', 'JPG', 'jpeg', 'JPEG', 'gif', 'GIF', 'png', 'PNG',
                    'bmp', 'BMP', 'ico', 'ICO', 'pdf', 'PDF', 'doc', 'DOC',
                    'rtf', 'RTF', 'txt', 'TXT', 'zip', 'ZIP', 'rar', 'RAR', 'docx', 'DOCX']
            },
            params = $.extend({}, defaults, settings);
        mainArea.parent().find(":file").on(App.event.click(), function (e) {
            if(typeof e.originalEvent !== "undefined"){
                e.preventDefault();
            }
        });
        var availableExtension = params.extensions;
        mainArea.on(App.event.click(), '.add-file.attach-file', function(e){
            e.preventDefault();
            if (!isSupportHTML5) {
                var countFile = $(this).siblings('input:file').length,
                    file = $(this).siblings('input:file:eq(0)'),
        ...
        if (!isSupportHTML5) {
            var uploadFileIframe = function (file) {
                ...
                iFrame.load(function () {
                    ...
                    if($('#' + nameIfrmae).contents().find("body form").length === 0){
                        ...
                        if ($.isPlainObject(resp)) {
                            ...
                            insertLocation.apply(currFilesBlock, [resp]);
        ...
        var insertLocation = function(response){
            $(this).find('input[name*="[location]"]').val(response);
        };

        var uploadFileHTML5 = function (file, currFilesBlock) {
            var formdata = new FormData();
            formdata.append('file', file);
            formdata.append('extensions', params.extensions);
            $.ajax({
                url: App.config.basePath + '/attachments/upload/ajax',  //Server script to process data
                type: 'POST',
                data: formdata,
                cache: false,
                contentType: false,
                    ...
                    if (myXhr.upload) { // Check if upload property exists
                        myXhr.upload.addEventListener('progress', function (evnt) {
                            if (evnt.lengthComputable) {
                                ...
                                currFilesBlock.find('.loaded').css('width', (Math.floor(done / total * 1000) / 10) + '%');
                            }
                        }, false);
                    }
                    return myXhr;
...
//                        currFilesBlock.find('.loaded').css('width', '100%');
                        currFilesBlock.find('.loaded').closest(".upload-progress").removeClass('upload-progress').addClass('check ico-small floatRight');
                        insertLocation.apply(currFilesBlock, [resp]);
                    }
                    unDisabledSubmitButton();
                });
        ...
        mainArea.on("change", ".form-file-upload, .comment-file-upload", function (e) {
            var self = $(this),
                form = self.closest('form'),
                attachmentBlock = self.closest('.attachments-block');
            submitButton = form.find('[type="submit"]');
            //submitButton.attr("disabled", "disabled").addClass("disabled");
            var length = !isSupportHTML5 ? 1 : this.files.length;

            for (var i = 0; i < length; i++) {
                var name = !isSupportHTML5 ? this.value : this.files.item(i).name;
                var filename = name.replace(/^.*[\/\\]/g, "");
                var ext = filename.substring(filename.lastIndexOf('.') + 1);
                if ($.inArray(ext, availableExtension) >= 0) {
                    ...
                    var existFileRow = attachmentBlock.find('.inner-files'),
                        ...
                        newFileRow = '<div class="inner-files"><a href="#" class="ico-small idelete">#</a><input type="hidden" name="fileinfo[' + index + '][name]" value="' + filename + '"><input type="hidden" name="fileinfo[' + index + '][location]"><span class="filename">' + filename + '</span></div>';
                    if(self.attr("multiple") !== "multiple" && index !== 1){
                        existFileRow.replaceWith(newFileRow);
                    }else{
                        attachmentBlock.append(newFileRow);
                    }
                    var currFilesBlock = attachmentBlock.find('.inner-files:has(input[name="fileinfo[' + index + '][name]"])');
                    currFilesBlock.find('span').after(progress);
                    if (isSupportHTML5) {
                        uploadFileHTML5(this.files.item(i), currFilesBlock);
                    }else{
                        currFilesBlock.addClass('curr');console.log(self.attr('id', 'file-' + index));
                        uploadFileIframe(self.attr('id', 'file-' + index));
                    }
        ...
        var unDisabledSubmitButton = function(){
            submitButton.removeAttr("disabled").removeClass("disabled");
        };

        mainArea.on("click", ".idelete", function (e) {
            e.preventDefault();
            if ($(this).attr("href") !== '#') {
                App.remote.get($(this).attr("href"), {format: 'json'}, function (r) {
                    var result = App.remote.parseResult(r);
            ...
            return $(this).closest('.inner-files').remove();
        });
...
/** File: application/3.0.2/modules/resources/assets/desktop/js/comments.js **/
...
$(function () {
    ...
    mainArea.attachFiles();
    mainArea.on("submit", '#comment-add', function (e) {
        ...
        var form = $(this),
            ...
            comment_content = form.closest('.leave-comment-content'),
            commText = form.find("#comment_text"),
        ...
        if (commText.val().length < 3) {
            App.notify.error(App.t("Comment should be at least 3 characters!"));
            return false;
        }

        var anonymousCommenting = App.getParam('anonymousCommenting', false);

        if (App.loggedUser().isGuest() && !anonymousCommenting) {
            var params = {'form_id': $(this).attr("id")};
            App.openLoginPopup(params);
            return false;
        }

        if (anonymousCommenting){
            var anonymousEmail = $('#anonymous_email_external');
            var anonymousName = $('#anonymous_name_external');

            var pattern = /^[\w.\-_]+@[\w.\-_]+$/;
            if (anonymousEmail.val().trim() == '' || !pattern.test( anonymousEmail.val().trim() )) {
                anonymousEmail.addClass('error');
                App.notify.error( App.t('Email is incorrect!') );
                return false;
            } else {
                anonymousEmail.removeClass('error');
            }

            $('#anonymous_email').val( anonymousEmail.val().trim() );
            $('#anonymous_name').val( anonymousName.val().trim() );
        }
        ...
        $.ajax({
            ...
            if (result.data.getOutput()) {
                var html = result.data.getOutput();
                var textAreaId = $(html).find("textarea").attr("id");
                var siblingsReply = wrapper.siblings("[reply_to=" + replyToId + "]");
                if (siblingsReply.length) {
                    var newComment = $(html);
                    wrapper.siblings("[reply_to=" + replyToId + "]").last().after(newComment);
                    App.utils.scrollTo(newComment);
                } else {
                    wrapper.before($(html));
                }
                ...
                var commentsStat = $('#statistics-comments'),
                    commentsCounter = $("#count-comments"),
                    count;
                if (commentsStat.is('div')) {
                    count = commentsStat.text() * 1 + 1;
                    commentsStat.html(count);
                } else if (commentsCounter.is('span') || commentsCounter.is('div')) {
                    count = commentsCounter.html() * 1 + 1;
                } else {
                    count = 1;
                }

                commentsCounter.html(count);
                $("#statistics-participants").html(result.data.getParam("participants"));
            }

            if (form.find("#vote").val() !== 0) {

                if (result.data.getParam("negativeVotes") !== null) {
                    $(".button-object-full.iDont .how-vote").html(result.data.getParam("negativeVotes"));
                }

                if (result.data.getParam("positiveVotes") !== null) {
                    $(".button-object-full.iLikeIt .how-vote").html(result.data.getParam("positiveVotes"));
                }
                var disactive = $(".buttons-object-full .disactive");
                if (disactive.length) {
                    $(".button-object-full:not(.disactive)").addClass("disactive");
                    disactive.removeClass("disactive");
                } else {
                    var votes = $(".title-object-full .votes center");
                    votes.html(votes.text().trim() * 1 + 1);
                }
            }
            form.find("textarea, input[type=text]").val('');
            form.find("iframe").contents().find("body").html("");
            form.find("input[type=checkbox], input[type=radio]").removeAttr('checked');
            wrapper.find('input[name=is_negative]').change();
            form.find("#vote").val('0');
            form.find('input[name="comment_image[]"]').remove();
            form.find('.inner-files').remove();
            if (wrapper.hasClass('comment')) {
                wrapper.remove();
            }

            if ($('.comment.comment-reply-to[reply_to="'+replyToId+'"]').length >= 1) {
                $('.comment.comment-reply-to[reply_to="'+replyToId+'"]:last').next('.comment-quick-reply').removeClass('hidden');
            }
    ...
    var canManage = App.getParam('commentOptionsCount', null) == null ? false : App.getParam('canManage');
    var treatCommentsOptions = function (wrapper) {
        if (App.getParam('commentOptionsCount', null) == null) {
            return false;
        }
        ...
        if (canManage) {
            ...
            wrapper.find('.comment-options-toggle a.options').show().off('click').on('click', function () {
                ...
                parent.find('.comment-option').show(100);
                if (isReply) {
                    parent.find('a.cancel').show();
                } else {
                    parent.find('a.cancel').hide();
                }
                $(this).hide();
                return false;
            });
    ...
    treatCommentsOptions($('.leave-comment-block form'));

    mainArea.on('click', ".comment-reply", function (e) {
        e.preventDefault();
        ...
        var user = commentBlock.find(".comment-content-info").find('a.user_id'),
            ...
            preloader = $('.comment-preloader').clone(true);
        ...
        commentForm.find('h2').remove();

        var prevBlock = lastReply.length > 0 ? lastReply : commentBlock;

        prevBlock.after(preloader.addClass('comment').addClass('comment-reply-to'));
        prevBlock.after(commentForm.addClass('comment').addClass('comment-reply-to'));
        ...
        prevBlock.next(".leave-comment").find('#comment_text').click();

        $('.leave-comment.comment-reply-to').find('form').each(function () {
            treatCommentsOptions($(this));
        });

        setTimeout(function () {
            var ew    = commentBlock.next().find('.textarea-editor').get(0).contentWindow.window,
                range = ew.getSelection().getRangeAt(0);
            $(range.endContainer).focus().click();
        }, 100);
    });

    mainArea.on(App.event.click(), '.comment-quick-reply input', function (e) {
        var replyButton = $('div.comment[comment_id="'+$(this).attr('data-comment-id')+'"]').find('a.comment-reply');
        replyButton.data('stop-scrolling', 1).trigger('click');
    });

    /* Vote for comment */
    mainArea.on('click', ".rating-response", function (e) {
        ...
        e.preventDefault();
        if (!App.getParam("canVote")) {
            var params = {'get': {}, 'redirect': ratingResponse.find('a').attr("href")};
            App.openLoginPopup(params);
            return false;
        }
        var comment = ratingResponse.closest('.comment');

        ratingResponse.toggleClass('voted-comment');
        comment.find(".preloader").show();
        App.remote.get(ratingResponse.find("a").attr("href"), {format: 'json'}, function (r) {
            var result = App.remote.parseResult(r);
            ...
            if (ratingResponse.hasClass("voted-comment")) {
                votes.text(parseInt(votes.text(), 10) + 1).addClass("now-vote").append("<i></i>").show();
                setTimeout(function () {
                    votes.removeClass("now-vote");
                }, 5000);
            } else {
    ...
    //edit comment
    mainArea.on("submit", ".comment-edit-form", function () {
        var form = $(this);
        var comment = form.parents(".comment");
        var wrapper_height = comment.height();
        var preloader = comment.next($('.comment-preloader'));
        var commText = form.find("textarea");
        ...
        App.remote.post(form.attr('action'), App.getFormParams(form), function (r) {
            var result = App.remote.parseResult(r);
            ...
            comment.hide();
            var html = result.data.getOutput();
            var text = $(html).find(".comment-content-text");
    ...
    mainArea.on("click", ".edit-comment", function () {
        ...
        var commentContentVal = $.trim(wysiwyg.htmlToBbcode(comment.find('.comment-content-text'))),
            textAreaId = 'comment_text_edit-' + commentId,
            preloader = $('.comment-preloader').clone(true).addClass('comment');

        if (comment.hasClass('comment-reply-to')) {
            comment.after(preloader.addClass('comment-reply-to'));
        } else {
            comment.after(preloader);
        }
        //initialise wysiwyg
        commentContentText.html('<div class="description-object"><div class="resizer-wrap"><textarea id="' + textAreaId + '" rows="3" cols="3" name="content" style="display: none;">' + commentContentVal + '</textarea></div></div>');
        commentContentText.find('.description-object').append($('.allowed-codes-block:eq(0)').clone(true));
        var textArea = comment.find('textarea');
        App.initEditor(textArea);
        //create an edit form
        var url = App.config.basePath + '/comments/' + commentId + '/edit',
        ...
        filesOld.find('a').each(function () {
            form.find(".attachments-block").append('<div class="inner-files"><a class="ico-small idelete" href="' + $(this).attr('href') + '/delete' + '">#</a><span class="filename">' + $(this).attr('title') + '</span></div> ')
        });

        filesOld.closest('.files-comment').remove();
    ...
    mainArea.on("click", ".delete-comment", function () {
        var comment = $(this).closest('.comment'),
            replyToId = comment.attr('comment_id');
        comment.find(".preloader").show();
        App.remote.get(this.href, {format: 'json'}, function (r) {
            comment.find(".preloader").hide();
            var result = App.remote.parseResult(r);
            ...
            $("#count-comments").html(mainArea.find('.comment:not(.trashed):not(.best-answer):visible').length);
        });
        return false;
    });
    ...
    mainArea.on("click", ".permalink", function () {
        location.href = $(this).attr("href");
        App.utils.scrollTo($(this).closest(".comment"));
        return false;
    });
    ...
    if (hash.indexOf('comment-') > 0) {
        App.utils.scrollTo($('[comment_id=' + hash.substr(9) + ']'));
    }
});


/** File: application/3.0.2/assets/desktop/js/textareaResizer.js **/
...
function textareaResizer(e) {
    ...
    textareaEditor = document.getElementById(
        resizer.id.substr(0, resizer.id.length - 8) + '_editor'
    );
    ...
    $(document).mouseup(textareaResizer_cleanup);
    return false;
}
...
function editorCommand(editor, command, options) {
    content = editor.contentWindow;
    try {
        if ($.browser.msie) {
...
wysiwyg = {
    ...
    bbcodeToHtml: function (txt) {
        ...
        text = text.replace(/\[h4\]/ig, '<h4 style="background-color:#aaaaaa; color:#ffffff; margin:-10px -18px 10px; padding: 3px 6px;">')
            ...
            .replace(/\[b\]/ig, '<span style="font-weight: bold;">')
            .replace(/\[\/b\]/ig, "</span>")
            ...
            //.replace(/\[video\](.*?)\[\/video\]/g, function (_str, p1, p2) { return '<video>' + p1 + '</video>'; });
            .replace(/\[video\](.*?)\[\/video\]/g, function (_str, p1, p2) {
                return '<img src="' + App.assets.common.image('video-player.png') + '" xtype="video" alt="' + p1 + '" />';
            })
    ...
    htmlToBbcode: function (el) {
        ...
        html.find('a').each(function () {
            $(this).replaceWith('[url=' + $(this).attr("href") + ']' + $(this).html() + '[/url]');
        });
        ...
        html.find('div, br').each(function () {
            $(this).replaceWith("\n" + $(this).html());
        });
    ...
    replaceSpan: function (el) {
        replaceOpenTags = replaceCloseTags = "";
        ...
        if (el.style.textDecoration.indexOf("underline") >= 0) {
            replaceOpenTags += "[u]";
            replaceCloseTags = "[/u]" + replaceCloseTags
        }
        if (el.style.textDecoration.indexOf("line-through") >= 0) {
            replaceOpenTags += "[s]";
...
function copyTo(body, textArea, n) {
    ...
    if (n == 1) {
        ...
        text = textArea.val();
        html = wysiwyg.bbcodeToHtml(text);
        body.html(html);
    }
}

var toVideoTag = function (_url) {
    var parseVideoURL = function (_url) {
        _url.match(/^(http|https):\/\/(?:.*?)\.?(youtube|vimeo)\.com\/(watch\?[^#]*v=(\w+)|(\d+)).+$/);
        return {
            provider: RegExp.$2,
            id: RegExp.$2 == 'vimeo' ? RegExp.$3 : RegExp.$4
        }
    };

    var _video = parseVideoURL(_url + '/'),
        fullyQualifiedUrl;

    if ('provider' in _video) {
        switch (_video.provider) {
            case 'youtube':
                fullyQualifiedUrl = 'http://www.youtube.com/v/' + _video.id;
                break;
            case 'vimeo':
                fullyQualifiedUrl = 'http://vimeo.com/moogaloop.swf?clip_id=' + _video.id + '&amp;force_embed=1&amp;server=vimeo.com&amp;show_title=0&amp;show_byline=0&amp;show_portrait=0&amp;color=e3d06f&amp;fullscreen=1&amp;autoplay=0&amp;loop=0';
                break;
        }
    }

    var tag = "<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' original='" + _url + "' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0'>";
    tag += "<param name='movie' value='" + fullyQualifiedUrl + "' />";
    tag += "<embed width='100%' height='266' src='" + fullyQualifiedUrl + "' type='application/x-shockwave-flash'>";
    tag += "</embed>";
    tag += "</object>";
...
$("document").ready(function () {
    ...
    App.initEditor = function (textArea) {
        var frame = $('<iframe name="textarea-editor" frameborder="0" src="javascript:true;" class="textarea-editor" >');
        frame.load(function () {
            ...
            head.append('<link type="text/css" rel="stylesheet" media="screen" href="' + App.assets.common.stylesheet('/texteditor.css') + '">');
            ...
            if (App.getParam('useWysiwyg') == 0) {
                ...
                textarea.hide();
                textareaEditor.show();
                container.find(".change-view i").addClass("view-on");

                contents.one(App.event.click(), function () {
                    var content = $(document.activeElement).parents(".description-object");
                    var codeBlock = content.find(".allowed-codes-block");
                    if (codeBlock.html().length) {
                        content.css("padding-top", "49px");
                        codeBlock.fadeIn();
                    }
                });
                ...
                body.keyup(function () {
                    ...
                    copyTo($(this), textarea, 1);
                });
            }

        });
        frame.attr("id", $(textArea).attr("id") + "_editor");
        $(textArea).after(frame);
    };

    $("body").on(App.event.click(), 'a.code', function () {
        var container = $(this).parents(".description-object"),
        ...
        switch (tag) {
            ...
            case 'source':
                if (viewOn) {
                    ...
                    wrapTag(textArea.get(0), '[co][li]', '[/li][/co]');
                }
                break;
                ...
                if (viewOn) {
                    ...
                    if (selectedText !== "") {
                        var regexp = new RegExp('^(https?:\/\/[^\s]+)|(www?\.[^\s]+)', 'g'),
                            text = "";
                        if (selectedText.match(regexp)) {
                            text = selectedText;
                        }
                        url = prompt(App.t('Enter link address:'), text);
                        if (url) {
            ...
            case 'image':
                if (viewOn) {
                    ...
                    if (!App.loggedUser().isGuest() && !imagesList.is(":visible")) {
                        image.addClass('allowed-code-image');
                        imagesList.show(100);
                        /* if we click in an another place 'imagesList' will be hidden */
                        $('body').mouseup(function (e) {
                            if (!$.inArray(e.target.className, ['code upload_image', 'code link-to-image'])) {
                                hideImagesList();
                                $('body').off('mouseup');
                            }
                        });
                    ...
                    imagesList.off();
                    var insertImage = function (link, basename) {
                        ...
                        if (viewOn) {
                            if (url) {
                                ...
                                textAreaEditor.contents().find("body img").css("max-width", "100%");
                            }
                            copyTo(textAreaEditor.contents().find("body"), textArea, 1);
                            if (basename) {
                                textAreaEditor.closest('form').append('<input type="hidden" value="' + basename + '" name="textarea_images[]">');
                            }
                        } else {
                    ...
                    imagesList.one(App.event.click(), '.link-to-image', function (e) {
                        ...
                        if (linkToImage && linkToImage.length) {
                            insertImage(linkToImage, null);
                        }
                        hideImagesList();
                    });
                    if (App.loggedUser().isGuest()) {
                        imagesList.find('.link-to-image').click();
                    }
            ...
            case 'video':
                if (viewOn) {
                    var url = prompt(App.t('Enter Video Link:'));
                    if (url != null && url != "") {
                        var video = toVideoTag(url);
                        if (!video || '' == video.provider) {
                            alert('Incorrect Video URL!');
                            return;
                        }
                        video = '<img src="' + App.assets.common.image('video-player.png') + '" xtype="video" alt="' + url + '" />';
                        editorCommand(textAreaEditor.get(0), 'Inserthtml', video);
                        textAreaEditor.contents().find("body object").css("max-width", "100%");
                    }
                } else {
                    wrapTag(textArea.get(0), '[video]', '[/video]');
                }
                break;
            case 'spoiler':
                if (viewOn) {
                ...
                var spoilerName = prompt(App.t('Enter a Title for the Spoiler'));
                        ...
                        textAreaEditor.contents().find("ul").each(function () {
                            ...
                            if ($(this).attr('class') == '' || $(this).attr('class') == undefined) {
                                ...
                                if (!$(this).find('.spoiler-title').length) {
                                    $(this).prepend('<li><span class="spoiler-title" style="color:#004D99;border-bottom:1px dotted;text-decoration:none;">' + spoilerName + '</span></li>');
                                }
                            }
                        });
                    } else {
            ...
            case 'align_right':
                ...
                var hideParent = function () {
                    parent.children(':not(.alignes-first)').hide(100);
                    parent.removeAttr('style');
                };
                if (align.next().length && !align.next().is(":visible")) {
                    align.siblings().show(100);
                    parent.css('background', '#eaf2f3');
                    $('body').mousedown(function (e) {
                        if (!$.inArray(e.target.className, ['align', 'align_left', 'align_center', 'align_right']) < 0) {
                            hideParent();
                            $('body').off('mousedown');
                        }
                    });
...
/** File: application/3.0.2/modules/system/assets/desktop/js/report_abuse.js **/
...
App.onReady(function(){
    ...
    $('#topic-report-abuse').on(App.event.click(), function () {
        ...
        var element = $(this);
        var wrapper = element.parents('p:first');
        element.hide();
        wrapper.append('<i class="preloader ico-small"></i>');
