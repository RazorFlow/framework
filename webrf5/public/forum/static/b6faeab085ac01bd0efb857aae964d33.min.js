...
/** File: application/3.0.2/modules/helpdesk/assets/desktop/js/frontend.js **/
            ...
            ticket: {
                add: function () {
                    App.onReady(function () {
                        ...
                        if (subscribersBlock.length) {
                            var subscribe = function (id) {
                                ...
                                if (this.checked) {
                                    ...
                                    parent.removeClass('not-set');
                                    elemImg.data('powertip', App.t('Notify and Subscribe'));

                                } else {
                                    hidden.val(0);
                                    label.attr('for', isImgClick ? id : this.id);
                                    parent.addClass('not-set');
                                    elemImg.data('powertip', App.t('Not Set'));
                                    markResponsible.call($(this).siblings('label')[0], true);
                                }
                            ...
                            subscribersBlock
                                ...
                                .on(App.event.click(), '.notifications-link label', markResponsible);
                        ...
                        mainArea.on('change', '#another_author', function () {
                            mainArea.find('.specified-author-block').toggle();
                        });
                        ...
                        mainArea.on('submit', 'form', function (e) {
                            ...
                            // email validation
                            var contactEmail = $('#notify_email');
                            if (contactEmail.is(':visible')) {
                                var pattern = /^[\w.\-_]+@[\w.\-_]+$/;
                                if (!pattern.test(contactEmail.val().trim())) {
                ...
                show: function () {
                    App.onReady(function () {
                        ...
                        status.on(App.event.click(), function (e) {
                            ...
                            if (!statusList.length || parseInt(status.data('is-completed'))) {
                                return;
                            }
                        ...
                        /* Set Responsible */
                        if ($('.wrap-container').find('.object_manage_responsible').hasClass('can-manage')) {
                            App.manageResponsible({module: 'helpdesk', view: '_ticket_user_info.phtml'}, function (response) {
                                var userInfo = response.data.getOutput();
                                mainArea.find('.ticket-user-info').replaceWith(userInfo);
                            });
                        }
                        ...
                        $(".wrap-container").on(App.event.click(), ".object_complete", function (event) {
                            ...
                            link.removeClass('active').html('<i class="preloader ico-small"></i>');

                            App.remote.post(this.href, {}, function (r) {
                                link.addClass('active').html(linkCaption);
                                ...
                                links.prop('href', result.data.getParam('href'));
                                links.text(result.data.getParam('title'));
                                status.data('is-completed', result.data.getParam('is_completed'));
                                App.params['state'] = result.data.getParam('is_completed');

                                $('div.comment-change-state').find('select').val(result.data.getParam('is_completed'));
                            ...
                            App.dynamic.modal.openFromUrl({
                                ...
                                complete: function (modal) {
                                    ...
                                    content.on(App.event.click(), '.sharing-submit', function (event) {
                                        ...
                                        App.remote.post(button.attr('href'), {}, function (r) {
                                            preloader.hide();
                        ...
                        App.dynamic.event().bind('comment.created', function (event, result) {
                            ...
                            if (ticketStatus != null) {
                                var res = $.parseJSON(ticketStatus);
                                applyStatusPrams(res, result.data.getParam('statusId', undefined));
                                $('.ticket-status .content-object-status').data('is-completed', result.data.getParam('is_completed'));
                                App.params['state'] = result.data.getParam('is_completed');
                ...
                list : function () {
                    App.onReady(function () {
                        ...
                        $('.reload-tickets').on(App.event.click(), function () {
                            var button = $(this);
                            button.addClass('hidden').next().addClass('ico-small');

                            App.remote.get(button.attr("href"), {format: 'json'}, function (r) {
                                button.removeClass('hidden').next().removeClass('ico-small');
                                var result = App.remote.parseResult(r);
                        ...
                        $('#ticket-search-form').on('submit', function (e) {
                            e.preventDefault();
                            $('.filter-area').find('input.ticket-number').val(
                                $(this).find('input[name="search_phrase"]').val()
                            ).trigger('change');
                            return false;
                        });

                        $.helpdeskMassEdit(mainArea, massEditLeft);
                    });
                }
            }
        }
...
/** File: application/3.0.2/modules/helpdesk/assets/desktop/js/paginator.js **/
...
App.onReady(function () {
    ...
    //url = url.replace(/(.*)\/category\/(\w+)\/sort\/(\w+)\/page\/(\d+).*/, '$1');

    var filtersArea = $(".filter-area");

    var paginatorButton = $('.paginator').find(' li span.cur');
    var globPage = paginatorButton.length > 0 ? parseInt(paginatorButton.text()) : 1;
    ...
    var pushNewState = function (page) {
        ...
        //var sort = data['status'] != undefined ? data['status'] : 'popular';
        //window.history.pushState(document.title, document.title, url + '/category/'+categorySlug+'/sort/'+sort+'/page/'+data.page);
    };

    filtersArea.find('select').on('change', function () {
        pushNewState(globPage);
    });
